      ...
      for (const serverId of selectedServers) {
        ...
        if (isComplex) {
          ...
          while (tries-- > 0) {
            ...
            // Check if this is a progress response (has last_update field)
            if (data.last_update !== undefined) {
              // This is a progress response
              if (data.output && data.output !== lastOutput) {
                updateOutputPanel(panelId, data.output, 'running');
                lastOutput = data.output;
              } else if (!data.output && lastOutput === "") {
                // Initial progress with no output yet
                updateOutputPanel(panelId, "Команда запущена, очікування виводу...", 'running');
              }
              // Continue polling
              await new Promise(r => setTimeout(r, 2000)); // Poll every 2 seconds
            } else if (data.stdout !== undefined || data.stderr !== undefined) {
              // Final result received
              const output = data.stdout || data.stderr || "Команда завершена";
          ...
          while (tries-- > 0) {
            ...
            if (data.status === "no_result") {
              ...
              // Final result received
              const output = data.stdout || data.stderr || JSON.stringify(data);
              updateOutputPanel(panelId, output, 'success');
              break;
            } else {
              // Unexpected response
