...
def send_progress_update(command_id, output_chunk="", is_complete=False, stderr="", code=0):
    ...
    try:
        ...
        with open("agent_error.log", "a", encoding="utf-8") as f:
            f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Sending progress: {progress_data}\n")
        
        response = requests.post(progress_url, json=progress_data)
        response.raise_for_status()
        
        # Log the response
        with open("agent_error.log", "a", encoding="utf-8") as f:
            f.write(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Progress response: {response.status_code} - {response.text}\n")
...
def run_complex_command(command_id, command):
    ...
    try:
        ...
        send_progress_update(command_id, "Команда запущена...\n")
